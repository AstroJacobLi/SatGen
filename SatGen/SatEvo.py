...
flattening = 25.0  # disk scale radius / disk scale height
...
def loop(file):
    ...
    VirialOverdensity = np.zeros(mass.shape, np.float32) + 200.0
    MaxCircularVelocity = np.zeros(mass.shape, np.float32) - 99.0
        ...
        for id in idx:  # loop over branches
            ...
            for iz in np.arange(iza)[::-1]:  # loop over time to evolve
                ...
                if (fd > 0.0) and (k == 1):
                    ...
                    Rep = gh.Reff(Rvp, c2p)
                    adp = 0.766421 / (1.0 + 1.0 / flattening) * Rep
                    bdp = adp / flattening
                    Mdp = fd * Mp
                    p = [
                ...
                if m > cfg.Mres:
                    ...
                    m, lt = ev.msub(
                        s, p, xv, dt, choice="King62", alpha=StrippingEfficiency
                    )
    ...
    print(
        "    %s: %5.2f min, z50=%5.2f,fsub(>1e-4)=%8.5f,fstar=%8.5f"
        % (outfile, (time_end_tmp - time_start_tmp) / 60.0, z50, fsub, fstar)
    )
    # sys.exit() # <<< test
