...
z_grid = np.logspace(0.0, 2.0, 10000) - 1.0  # uniform in log(1+z)
...
while z <= zmax:
    ...
    zsample.append(z)
    dtsample.append(dt)
dtsample.append(0.0)  # append a zero to the end, making dtsample the same
# length as zsample
...
ures_grid = np.logspace(-6.0, 6.0, 1000)
...
# radius.
r_vals_int = np.logspace(-5.5, 1.0, nr)
...
for i in range(0, nr):
    fb_cs_interps_mass.append(
        RectBivariateSpline(log_fb_vals_int, log_cs_vals_int, gvdb_mm[:, :, i])
    )
    fb_cs_interps_sigma.append(
        RectBivariateSpline(log_fb_vals_int, log_cs_vals_int, gvdb_sm[:, :, i])
    )
    fb_cs_interps_d2Phidr2.append(
        RectBivariateSpline(log_fb_vals_int, log_cs_vals_int, gvdb_pm[:, :, i])
    )
...
for i in range(0, 3):
    ...
    for j in range(0, 3):
        V_by_V200c_interps[i].append(splrep(V_by_V200c_cdf[i, j], V_by_V200c_arr))
        Vr_by_V_interps[i].append(splrep(Vr_by_V_cdf[i, j], Vr_by_V_arr))

jiang_nu_boundaries = co.nu([5e12, 5e13], 0.0, **cosmo)
